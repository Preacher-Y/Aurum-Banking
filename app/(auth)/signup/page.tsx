  ...
  return (
    <main className="min-h-[100dvh] flex font-[family-name:var(--font-libertinus-serif-display)] bg-[#0d0b08] text-[#ede5d9]">
      ...
      >
        ...
        <div className="pointer-events-none absolute -bottom-40 -right-24 w-[380px] h-[380px] rounded-full bg-[#86642b] opacity-[0.03] blur-[110px]" aria-hidden="true" />
          ...
          {step === "credentials" ? (
            <>
              <div className="mb-10">
                ...
                <h2 className="text-[38px] font-normal leading-[1.1] text-[#ede5d9] mb-2">Join Aurum.</h2>
                <p className="text-[15px] tracking-[0.3px] text-[#8a7a6a]">Set up your private account today</p>
              </div>

              <form onSubmit={handleRegister} autoComplete="off" aria-label="Sign up form">
                <div className="space-y-3">
                  <div className="aurum-input-shell">
                    <div className="aurum-input-core">
                      ...
                      <input aria-label="Full name" autoComplete="name" className="aurum-input" placeholder="Full name" type="text" value={name} onChange={(e) => setName(e.target.value)} required />
                    </div>
                  </div>
                  ...
                  <div className="aurum-input-shell">
                    <div className="aurum-input-core">
                      ...
                      <input aria-label="Password" autoComplete="new-password" className="aurum-input" placeholder="Password" type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} style={{ paddingRight: "44px" }} required />
                  ...
                  <div className="aurum-input-shell">
                    <div className="aurum-input-core">
                      ...
                      <input aria-label="Confirm password" autoComplete="new-password" className="aurum-input" placeholder="Confirm password" type={showConfirm ? "text" : "password"} value={confirm} onChange={(e) => setConfirm(e.target.value)} style={{ paddingRight: "44px" }} required />
                      <button type="button" tabIndex={-1} onClick={() => setShowConfirm((v) => !v)} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#dcb96e] opacity-40 hover:opacity-70 transition-opacity duration-200" aria-label={showConfirm ? "Hide password" : "Show password"}>
                        {showConfirm ? (
                          <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
                            <path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24" /><line x1="1" y1="1" x2="23" y2="23" />
                          </svg>
                ...
                {error && <p className="text-[13px] text-[#e07b6a] tracking-wide mt-4 mb-1 pl-1">{error}</p>}

                <div className="mt-6">
                  <button className="aurum-cta-btn w-full disabled:opacity-60 disabled:cursor-not-allowed" type="submit" disabled={loading}>
                    <span className="tracking-[2.5px] uppercase text-[14px]">{loading ? "Creating…" : "Create Account"}</span>
              ...
              <div className="mt-10">
                ...
                <p className="text-center text-[14px] tracking-[0.3px] text-[#7a6a58]">
                  ...
                  <Link className="text-[#ca9340] hover:text-[#dcb96e] underline underline-offset-2 transition-colors duration-500" href="/login">Sign in</Link>
                </p>
              </div>
            </>
          ) : (
            <>
              <div className="mb-8">
                <div className="aurum-eyebrow mb-5">Verify Your Email</div>
              ...
              <form onSubmit={handleOtp} aria-label="OTP verification form">
                ...
                <button className="aurum-cta-btn w-full mt-6 disabled:opacity-60 disabled:cursor-not-allowed" type="submit" disabled={loading || otp.length < 6}>
                  <span className="tracking-[2.5px] uppercase text-[14px]">{loading ? "Verifying…" : "Verify & Continue"}</span>
                  <span className="aurum-cta-icon">
                    <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M5 12h14M12 5l7 7-7 7" /></svg>
                  </span>
                </button>
